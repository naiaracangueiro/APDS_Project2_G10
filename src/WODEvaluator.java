...
public class WODEvaluator {
    ...
    /**
     * Evaluates a configuration and returns the total quest value.
     * Uses the config array to determine which quests are selected.
     * @param allQuests the complete list of quests
     * @param config binary array where config[i]=1 means quest i is selected
     * @param maxTime maximum allowed time in minutes
     * @return total quest value (Legendary=5, Rare=2, Common=1), or -1 if invalid
     */
    public static int evaluate(List<Quest> allQuests, int[] config, int maxTime) {
        // Maps to track constraints as we process quests
        Map<LocalDate, Integer> questsByDeadline = new HashMap<>();
        Map<String, Integer> countBySubject = new HashMap<>();
        Map<String, Integer> timeBySubject = new HashMap<>();
        ...
        // Process only selected quests (config[i] == 1)
